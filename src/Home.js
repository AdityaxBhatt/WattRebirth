...
function Home() {
  ...
  const script = document.createElement('script');
    script.src = `https://maps.googleapis.com/maps/api/js?key=AIzaSyBdle4iPuZ4igzYtTiGA1N_ZSogWJ3ysV0&libraries=places,directions&callback=Function.prototype`;
    script.async = true;
  ...
  useEffect(() => {
    ...
    const findNearbyEwasteCenters = (location) => {
        const request = {
            location: location,
            radius: 10000,
            keyword: 'electronics waste recycling',
            type: 'recycling',
        };
